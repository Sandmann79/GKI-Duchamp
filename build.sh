...
GKI_RELEASES_REPO="https://github.com/Sandmann79/GKI-Duchamp"
...
if [[ "$KERNEL_REPO" == *"github.com"* ]]; then
    gh api "repos/$(echo $KERNEL_REPO | cut -d/ -f4-)/commits?sha=${KERNEL_BRANCH}&per_page=10" --jq '.[] | "- [" + .sha[0:7] + "](" + .html_url + ") " + (.commit.message | split("\n")[0])'\
...
gh api 'repos/ReSukiSU/ReSukiSU/commits?sha=main&per_page=10' --jq '.[] | "- [" + .sha[0:7] + "](" + .html_url + ") " + (.commit.message | split("\n")[0])'\
...
if [ "$KSU" = "KSU" ]; then
  ...
  if susfs_included; then
    ...
    git config --global user.email "sandmann79@gmx.net"
    git config --global user.name "Sandmann79"
    git add .
